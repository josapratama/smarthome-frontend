  ...
  return (
    <div className="space-y-6 p-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                {devicesQuery.isLoading ? (
                  ...
                  <>
                    <h3
                      className={`text-3xl font-bold mt-2 ${criticalDevices.length > 0 ? "text-orange-600" : "text-gray-400"}`}
                    >
              ...
              <div
                className={`h-12 w-12 rounded-full flex items-center justify-center ${criticalDevices.length > 0 ? "bg-orange-100" : "bg-gray-100"}`}
              >
                <AlertTriangle
                  className={`h-6 w-6 ${criticalDevices.length > 0 ? "text-orange-600" : "text-gray-400"}`}
                />
              </div>
            </div>
          </CardContent>
      ...
      {criticalDevices.length > 0 && (
        <Card className="border-l-4 border-l-orange-500 bg-gradient-to-r from-orange-50 to-transparent shadow-md">
          <CardContent className="p-6">
                ...
                <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                  {criticalDevices.slice(0, 3).map((device) => (
                    <Link
                      ...
                      className="block"
                    >
                      <div className="p-4 rounded-lg bg-white border border-orange-200 hover:border-orange-300 hover:shadow-md transition-all">
          ...
          {devicesQuery.isLoading ? (
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
              {filteredDevices.map((device) => {
                const { status, color, label } = getLastSeenStatus(
                  device.lastSeenAt,
                );
